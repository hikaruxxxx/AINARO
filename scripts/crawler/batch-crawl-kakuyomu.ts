...
// カクヨム一括クロール
// 使い方:
//   npx tsx scripts/crawler/batch-crawl-kakuyomu.ts data/targets/kakuyomu_stratified.json
//   npx tsx scripts/crawler/batch-crawl-kakuyomu.ts data/targets/kakuyomu_stratified.json --max-ep 10 --max-works 20

import fs from "fs/promises";
import path from "path";
import { crawlKakuyomu, closeBrowser } from "./kakuyomu";
...
function parseArgs() {
  ...
  if (args.length === 0) {
    console.log("使い方: npx tsx scripts/crawler/batch-crawl-kakuyomu.ts <targets.json> [--max-ep N] [--max-works N]");
    process.exit(1);
  }
...
async function main() {
  ...
  console.log(`🕷️ カクヨム一括クロール`);
  console.log(`  作品数: ${targets.length}`);
  if (maxEp) console.log(`  各作品上限: ${maxEp}話`);
  console.log("");

  // 進捗ログ
  const logPath = path.join(outputDir, "_kakuyomu_crawl_log.json");
  ...
  try {
    for (const target of targets) {
      ...
      try {
        await crawlKakuyomu(target.ncode, { endEp: maxEp, outputDir });
        log[target.ncode] = { status: "done", completedAt: new Date().toISOString() };
        completed++;
      } catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
        console.error(`❌ 失敗: ${msg}`);
        log[target.ncode] = { status: "failed" };
        failed++;
      }
