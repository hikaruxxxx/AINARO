#!/usr/bin/env python3
"""
ヒット予測モデル v12-ep1 — ep1 のみで訓練・ep1 のみで予測（58k スケールアップ版）

v11-ep1 との差分:
- 訓練データを 20,077 → 37,500+ に拡大（narou 58k 全件 ep1 抽出）
- パラメータは v11-ep1 と同一（データ増量効果の切り分けのため）

設計思想（v11-ep1 から継承）:
- ep1 のみで特徴抽出→訓練→予測で一貫（推論との分布整合）
- リーケージ対策として log_episodes / genre_group は除外
"""
...
model_output = {
    ...
    "description": "ヒット予測モデル v12-ep1 — ep1 訓練・予測 (58k narou 全件スケールアップ)",
...
output_path = MODELS / "hit-prediction-v12-ep1.json"
