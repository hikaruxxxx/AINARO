#!/usr/bin/env python3
"""
ヒット予測モデル v11-ep1 — ep1 のみで訓練・ep1 のみで予測（分布整合版）

設計思想:
- v11-pure は 10話平均特徴で訓練していたが、推論時 (screen-mass) は ep1 のみ
- この分布不一致でヒット作も bottom 判定される問題を修正
- v11-ep1 は ep1 のみで特徴抽出→訓練→予測で一貫させる
- リーケージ対策として log_episodes / genre_group は除外（v11-pure と同じ）
"""
...
model_output = {
    ...
    "description": "ヒット予測モデル v11-ep1 — ep1 のみで訓練・予測 (推論との分布整合版)",
...
output_path = MODELS / "hit-prediction-v11-ep1.json"
