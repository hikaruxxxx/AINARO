#!/usr/bin/env python3
"""
ヒット予測スクリプト v12 — アンサンブル (v12-ep1 × v12-longform)

v12-ep1: GP/ep top20% を予測する従来型のヒット確率 (AUC 0.746)
v12-longform: totalEpisodes>=50 で長編化を予測 (Top 5% Precision 69.5%)

両モデルの幾何平均を合算スコアとする。
- hit_ep1: 短期的な人気 (読者初速)
- hit_long: 長編継続力 (最終的な作品サイズ)
- ensemble: 両方高い=真のヒット候補
...
def load_model(version: str) -> dict:
    path = MODELS_DIR / f"hit-prediction-{version}.json"
    if not path.exists():
        print(f"エラー: モデルが見つかりません: {path}", file=sys.stderr)
        sys.exit(1)
    with open(path) as f:
        return json.load(f)


def score_with(model: dict, surface: dict, title: str, avg_ep_chars: int) -> float:
    """モデルの feature_names 順に特徴量を並べて予測確率を返す"""
    feat_sources = {**surface, **title_features(title), "avgEpChars": avg_ep_chars}
    features = []
    for name in model["feature_names"]:
        v = feat_sources.get(name)
        features.append(float(v) if v is not None else None)
    return predict_hit(features, model)


def load_genre_weights() -> dict:
    path = MODELS_DIR / "genre-weights-v12.json"
    if not path.exists():
        return {}
    with open(path) as f:
        return json.load(f)
...
GENRE_RE = re.compile(r"-\s*ジャンル[：:]\s*(\S+)")


def extract_genre(slug: str, explicit: str | None) -> str:
    if explicit:
        return explicit
    settings = CONTENT_DIR / slug / "_settings.md"
    if not settings.exists():
        return ""
    m = GENRE_RE.search(settings.read_text(encoding="utf-8"))
    return m.group(1) if m else ""


def resolve_weight(genre_slug: str, weights_doc: dict) -> dict:
    """ジャンルスラグ（自社）または narou ジャンル名から重みを解決"""
    if not genre_slug or not weights_doc:
        return {**weights_doc.get("default", {"w_ep1": 0.5, "w_long": 0.5}), "source": "default"}
    aliases = weights_doc.get("slug_aliases", {})
    if genre_slug in aliases:
        w = aliases[genre_slug]
        return {"w_ep1": w["w_ep1"], "w_long": w["w_long"], "source": f"alias:{w['alias_of']}"}
    narou = weights_doc.get("narou_genres", {})
    if genre_slug in narou:
        w = narou[genre_slug]
        return {"w_ep1": w["w_ep1"], "w_long": w["w_long"], "source": f"narou:{genre_slug}"}
    return {**weights_doc.get("default", {"w_ep1": 0.5, "w_long": 0.5}), "source": "default:unknown"}
...
def main():
    ...
    parser.add_argument("--genre", default=None, help="ジャンル（未指定時は _settings.md から自動取得）")
    ...
    model_ep1 = load_model("v12-ep1")
    model_long = load_model("v12-longform")

    p_ep1 = score_with(model_ep1, surface, title, avg_ep_chars)
    p_long = score_with(model_long, surface, title, avg_ep_chars)

    # 等重み幾何平均（従来）
    p_ensemble_equal = math.sqrt(p_ep1 * p_long)

    # ジャンル別加重幾何平均: p_ep1^w_ep1 * p_long^w_long
    genre = extract_genre(args.slug, args.genre)
    weights_doc = load_genre_weights()
    weight = resolve_weight(genre, weights_doc)
    p_ensemble_weighted = (p_ep1 ** weight["w_ep1"]) * (p_long ** weight["w_long"])

    def tier_of(p: float) -> str:
        # v12-ep1 の分布に基づく閾値（v11-ep1 と同等）
        if p >= 0.45: return "top"
        if p >= 0.35: return "upper"
        if p >= 0.25: return "mid"
        if p >= 0.15: return "lower"
        return "bottom"

    result = {
        "slug": args.slug,
        "episode": args.episode,
        "modelVersion": "v12-ensemble-weighted",
        "hitProbability": round(p_ensemble_weighted * 100, 1),
        "hitProbabilityEqual": round(p_ensemble_equal * 100, 1),
        "hitProbabilityEp1": round(p_ep1 * 100, 1),
        "hitProbabilityLongform": round(p_long * 100, 1),
        "tier": tier_of(p_ensemble_weighted),
        "tierEp1": tier_of(p_ep1),
        "tierLongform": tier_of(p_long),
        "genre": genre,
        "genreWeight": weight,
