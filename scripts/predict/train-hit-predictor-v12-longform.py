#!/usr/bin/env python3
"""
ヒット予測モデル v12-longform — ep1 の表層特徴から「長編化（50話以上続くか）」を予測

仮説: 長編＝ヒット。短期的なGP/epとは別軸で、読者を長く繋ぎ止められる作品か。
GP/ep top20% モデル (v12-ep1) と独立な代理指標として並行運用する。

設計:
- 目的変数: totalEpisodes >= LONGFORM_THRESHOLD (=50) を 1
- 特徴量: v12-ep1 と同じ表層29次元（title/story 長含む）
- リーケージ対策: totalEpisodes 自体は特徴量から除外（当然）
- 評価: AUC + Top-N% precision（ep1 特徴で長編化を見分けられるか）
"""
...
# ─── 目的変数: 長編化（totalEpisodes >= 50） ───
print("\n" + "=" * 60)
print("Step 2: 目的変数構築（totalEpisodes >= 50 を長編=1）")
print("=" * 60)

LONGFORM_THRESHOLD = 50
...
for r in feat_data:
    ...
    if eps > 0:
        n_with_eps += 1
        if eps >= LONGFORM_THRESHOLD:
            longform_ncodes.add(r["ncode"])

# hit_threshold / hit_ncodes の変数名は後続ロジックでそのまま使う
hit_threshold = LONGFORM_THRESHOLD
hit_ncodes = longform_ncodes

print(f"totalEpisodes>0 の作品: {n_with_eps}件")
print(f"長編閾値: {LONGFORM_THRESHOLD}話以上")
print(f"長編作品数: {len(hit_ncodes)} ({len(hit_ncodes)/max(n_with_eps,1):.1%})")
...
for r in feat_data:
    ...
    # totalEpisodes / log_episodes / genre_group は意図的に除外（リーケージ対策）
...
model_output = {
    "version": "v12-longform",
    "type": "binary_classification_gbt",
    "description": "ヒット予測モデル v12-longform — ep1 の表層特徴から長編化(>=50話)を予測",
    "target": "is_longform (totalEpisodes >= 50)",
    "longform_threshold_episodes": LONGFORM_THRESHOLD,
    "trainingData": {
        "total": stats["ok"],
        "hits": stats["hit"],
...
output_path = MODELS / "hit-prediction-v12-longform.json"
