#!/usr/bin/env python3
"""
ヒット予測モデル v11-body — 本文のみ版（タイトル特徴も除外）

設計思想:
- v11-pure では titleLen/titleHasBracket が重要度上位（27%分）を占めていた
- タイトルは「恋愛系の長タイトル慣習」「【】使用慣習」というジャンル規範を含む
- v11-body はタイトル特徴も除外し、**本文25次元のみ**で予測する
- これが「本文の craft」だけで測れる本当のヒット予測性能

特徴量: 表層25D のみ = 計25D
  除外: log_episodes, genre_group, titleLen, titleHasBracket, titleHasTemplateKw, avgEpChars
  avgEpChars もタイトル/ジャンル規範と相関するため除外
"""
...
ALL_FEATURES = list(SURFACE_FEATURES)  # 表層25Dのみ、メタ系はすべて除外
...
for r in feat_data:
    ...
    # メタ特徴（titleLen/titleHasBracket/titleHasTemplateKw/avgEpChars）も除外
...
model_output = {
    ...
    "description": "ヒット予測モデル v11-body — 本文表層25Dのみ (log_ep/genre/title/avgEpChars全除外)",
...
output_path = MODELS / "hit-prediction-v11-body.json"
