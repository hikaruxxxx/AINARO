#!/usr/bin/env python3
"""
ヒット予測モデル v11 — 表層特徴のみ / 50kスケール版

設計思想:
- v10はLLMスコア込みで6,148件訓練、AUC≈0.74程度
- v11はLLM特徴を捨てて表層+メタのみに絞り、50k+スケールで訓練
- データ量で殴ることで表層特徴だけでもベースライン性能を出せるか検証
- LLM評価の再現性問題（メモリ: feedback_llm_eval_reproducibility）も回避

目的変数: GP/totalEpisodes が上位20%なら1（v10と同一定義）
モデル: LightGBM (binary classification)
特徴量: 表層21D + メタ4D + log_episodes + genre_group + char_density 4D = 計31D

実行: python3 scripts/predict/train-hit-predictor-v11-surface.py
"""
...
model_output = {
    ...
    "description": "ヒット予測モデル v11 — 表層+メタ特徴のみ / 50kスケール訓練",
...
output_path = MODELS / "hit-prediction-v11-surface.json"
