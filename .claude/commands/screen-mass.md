...
3. 結果を `data/generation/batches/{batch_id}/{slug}/ep001.md` に保存
   - **必ず本文の先頭に `# 第{num}話「{タイトル}」\n\n---\n\n` のヘッダーを付ける**
   - タイトル欠落は目次UIで「第1話」しか出ず読者体験を損なうため、例外なく必須

**パターン反映（品質下限担保）:**

サブエージェントは生成前に必ず以下を読み、指示に含める:
- `content/style/anti_patterns.md` — 避けるべきパターン（**常に適用**、下限担保）
- `content/style/learned_patterns.md` — 効くパターン（**80%は適用**、残り20%は探索枠としてスキップ）

探索枠は「乱数で判定してスキップ」ではなく、バッチ全体で positive 適用版と未適用版を **8:2 で振り分ける** ことで局所最適を避ける。anti_patterns.md は探索枠でも常に適用。
...
先頭行には必ず「# 第1話「（この話を象徴する8〜20字の日本語タイトル）」」を入れる。
次に空行、「---」、空行、本文と続ける。タイトルは本文と別に考え、
ネタバレしすぎず読者を惹きつける言葉を選ぶ。
...
【必ず避ける（anti_patterns.md から抜粋、下限担保）】
{anti_patterns.md の内容をここに展開}

【推奨パターン（learned_patterns.md、探索枠でない場合のみ）】
{learned_patterns.md の内容をここに展開。探索枠ならこのブロックは省略}
```

**保存時チェック:**
- ep001.md の先頭行が `# 第1話「...」` でなければ失敗扱いとし、追記指示で修正（再生成ではない）
- 生成クライアント側で regex `/^# 第\d+話「.+」$/` を検証
