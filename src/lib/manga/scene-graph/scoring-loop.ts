...
export function buildSceneCandidatePrompt(
  ...
  return [
    ...
    `6b. dialogue_plan.key_lines の **数** は scene の panel 数に応じて以下を満たすこと (2026-05-18 Sprint 22 案2 追加、商業 narou コミカライズ水準):`,
    `    - panel_range が 15 panel 以上: key_lines >= 6`,
    `    - panel_range が 8-14 panel: key_lines >= 4`,
    `    - panel_range が 4-7 panel: key_lines >= 3`,
    `    - panel_range が 1-3 panel: key_lines >= 2`,
    `    beat_type=action や mode=silence でも最低 2 件は必要 (短い掛け声 / 状況声 / システム音声で可)。`,
    `    key_lines は L04 storyboard 生成時に各 panel に分配されるため、ここでの量が dialogue 密度の上流規定値になります。`,
    `    実例: 主人公独白 / 同僚・敵・家族のセリフ / システム音声 (公社・ナビ・ID端末) / off-frame voice (TV・隣室・通行人) を混在させる。`,
    `    空 key_lines [] は生成段階での失敗扱い。理由なく 0 個にしてはいけません。`,
