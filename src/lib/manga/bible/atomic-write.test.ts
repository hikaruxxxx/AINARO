...
describe("writeSnapshotV3Atomic", () => {
  ...
  it("splitFacts=true で facts/ ディレクトリ分割書き込み", async () => {
    ...
    expect(await pathExists(path.join(tmpDir, "snapshot.json"))).toBe(true);
    ...
    const snapshot = JSON.parse(await fs.readFile(path.join(tmpDir, "snapshot.json"), "utf8")) as BibleSnapshotV3;
  ...
  it("splitFacts=false で snapshot.json 単一ファイル", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "ainaro-atomic-"));
    const v3 = createMinimalV3WithFacts();
  ...
  it("write 失敗時に rollback が効く", async () => {
    ...
    await fs.writeFile(path.join(tmpDir, "snapshot.json"), `${JSON.stringify(original, null, 2)}\n`, "utf8");

    const originalRename = fs.rename.bind(fs);
    const rename = vi.spyOn(fs, "rename");
    rename.mockImplementation(async (from, to) => {
      if (String(from).includes(".tmp-write") && String(from).endsWith("snapshot.json")) {
    ...
    const restored = JSON.parse(await fs.readFile(path.join(tmpDir, "snapshot.json"), "utf8")) as BibleSnapshotV3;
  ...
  it("checksum 不一致なら rollback", async () => {
    ...
    readFile.mockImplementation(async (filePath, options) => {
      if (!corrupted && String(filePath).includes(".tmp-write") && String(filePath).endsWith("snapshot.json")) {
        corrupted = true;
